...
def addServersToCluster(appsrvName, nodes, abIdent, serversPerNode):
   ...
   cluster = AdminConfig.getid('/Cell:ws-cell/ServerCluster:' + clusterName + '/')
   ...
   # 
   #---------------------------------------------------------
   print "addServersToCluster: saving config changes."
   AdminConfig.save()
   ...
   while ("%s" % (index)) <= nodes:
      ...
      if len(nodeSync) == 0:
         print "addServersToCluster: Error -- NodeSync MBean not found for name " + nodeName
         return 
      enabled = AdminControl.getAttribute(nodeSync, "serverStartupSyncEnabled")
      if enabled == "false":
         print "addServersToCluster: Invoking synchronization for node " + nodeSync + " because serverStartupSyncEnabled is set to false..."
...
if (len(sys.argv) != 4):
   ...
   print "addServersToCluster: this script requires 4 parameters: appserv name, "
   print "     number of nodes, cluster a/b identifier, number of"
   print "     servers per node"
   print ""
   print "e.g.:     addServersToCluster  solar 4 b 1" 

else:

   addServersToCluster(sys.argv[0], sys.argv[1], sys.argv[2], sys.argv[3])
